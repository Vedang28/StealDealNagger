...
export default function Dashboard() {
  ...
  return (
    <PageWrapper>
      <div className="px-6 py-8 max-w-none">
        <PageHeader title="Pipeline" label="pipeline" description={team?.name} />
        ...
        {chartData.length > 1 && (
          <div className="bg-[#161616] rounded-xl border border-[rgba(255,255,255,0.07)] p-5 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          {STAGES.map((stage) => {
            ...
            return (
              <div
                key={stage}
                className="bg-[#111] rounded-xl p-3 flex flex-col gap-3"
              >
                {/* Column header */}
                <div className="flex items-center justify-between px-1">
                  ...
                  <span className="w-6 h-6 rounded-full bg-[#1e1e1e] text-[#f0ede8] text-xs font-bold flex items-center justify-center">
                    {stageDeals.length}
                  </span>
                </div>
                ...
                <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-0.5">
                  {stageDeals.length === 0 ? (
                    <div className="bg-[#161616] rounded-lg border border-dashed border-[rgba(255,255,255,0.1)] py-8 text-center">
                      <Briefcase className="w-6 h-6 text-[#333] mx-auto mb-1.5" />
                    ...
                    stageDeals.map((deal) => (
                      <button
                        ...
                        className={`w-full text-left bg-[#161616] rounded-lg border-l-4 p-3.5 hover:-translate-y-0.5 transition-all duration-200 cursor-pointer ${
                          STATUS_BORDER[deal.stalenessStatus] ||
                          "border-l-[#222]"
                        }`}
                      >
                        <p className="text-sm font-semibold text-[#f0ede8] truncate leading-snug">
                          {deal.name}
                        </p>
        ...
        {staleDeals.length > 0 && (
          <div className="bg-[#161616] rounded-xl border border-[rgba(255,255,255,0.07)] overflow-hidden">
              ...
              <table className="w-full">
                  ...
                  <tr className="border-b border-[rgba(255,255,255,0.07)]">
                    {[
                      ...
                      <th
                        ...
                        className="text-left px-5 py-3 font-mono text-[0.65rem] uppercase tracking-[0.12em] text-[#555] cursor-pointer select-none hover:text-[#f0ede8] transition-colors"
                      >
                        <span className="inline-flex items-center gap-1">
                          {col.label}
                          <ArrowUpDown
                            className={`w-3 h-3 ${tableSortKey === col.key ? "text-[#e8a87c]" : "opacity-30"}`}
                          />
                        </span>
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[rgba(255,255,255,0.05)]">
                  {staleDeals.slice(0, 10).map((deal) => (
                    ...
                    >
                      <td className="px-5 py-3.5 text-sm font-medium text-[#f0ede8]">
                        {deal.name}
                      </td>
                      <td className="px-5 py-3.5">
                        <span className="px-2 py-0.5 rounded-md bg-[#1e1e1e] text-xs font-medium text-[#888]">
                          {deal.stage}
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-sm font-semibold text-[#f0ede8]">
                        {formatCurrency(deal.amount)}
                      </td>
                      <td className="px-5 py-3.5">
                        <StatusBadge status={deal.stalenessStatus} />
            ...
            {staleDeals.length > 10 && (
              <div className="px-5 py-3 border-t border-[rgba(255,255,255,0.07)] text-center">
                <Link
                  to="/deals?status=stale"
                  className="text-sm text-[#e8a87c] hover:text-[#d4956a] font-medium"
                >
